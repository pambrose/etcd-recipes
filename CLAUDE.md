...
- **`etcd-recipes-core/`** — the library itself. Source is organized by recipe under `io.etcd.recipes.*`:
...
The current development branch is named after the version (e.g. `0.11.0`). Library version lives in `gradle.properties` (`version=...`); Kotlin/dependency versions live in `gradle/libs.versions.toml`; bump both when releasing.
